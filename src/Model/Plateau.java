...
public class Plateau implements ObjetVisite{
	...
	public boolean poserUneCarte(Carte carte, int x, int y) {
		...
		//appel la m�thode poser une carte de la coordonee qui va associ� l'attribut carte de la coordonne a l'objet carte donn� en argument
	...
	public void decalerPlateau(int deltax, int deltay) {
			...
			for (int x = 0; x < 5; x++) {
				for (int y = 2; y >= 0 ; y--) {
					...
					if (coord.getCarte() != null) {
						...
						Coordonee newcoord = this.recupererCoord(coord.getPositionX()+deltax,coord.getPositionY()+deltay);
						if (newcoord != null) {
		...
		while(it.hasNext()) {
			...
			if (coord.getCarte() != null) {
				Carte carte = coord.getCarte();
				Coordonee newcoord = this.recupererCoord(coord.getPositionX()+x,coord.getPositionY()+y);
				if (newcoord != null) {
					coord.enleverLaCarte();
					newcoord.poserUneCarte(carte);
				}
	...
	public int compterLesPoints(Carte carteVictoire) {
		...
		//pour chaque carte de la ligne on regarde si elle correspond a une caract�ritique de la carte victoire
		//si oui on incr�mente le nombre de carte d'affil� qui ont cette caract�ritique
		//si non on repart de 0
		//a la fin on regarde quel est le maximum de nbReference ce qui d�termine le nombre de carte d'affill� possedant une caract�ritique de la carte victoire
	...
	public int nombreDeCarteDansMains() {
		...
		while(it.hasNext()) {
			...
			//chaque joueur prend 2 carte puis a chaque tour il en pioche une nouvelle il aura donc le choix entre 3 cartes a chaque tours
	...
	public void jouer(boolean modeAvance) {
		...
		while ( ( (this.paquet.getNombreDeCarte() > 0) || (!this.isPlateauRempli()) ) || (this.nombreDeCarteDansMains() > 0) ) {
			...
			while (!joue) {
				...
				//on r�cup�re s�par�ment les coordon�es en splitant la chaine de caract�re a la virgule et ron r�cup�re chacune des 2 parties dans des variables
				try {
	...
	public static void main(String[] args) {
		...
		/*Carte triangleRougeVide = plat.paquet.recupererCarte("triangle", "rouge", false);
		Carte rondRougeVide = plat.paquet.recupererCarte("rond", "rouge", false);
		//Carte carreRougeVide = plat.paquet.recupererCarte("carre", "rouge", false);
		//Carte carreRougePlein = plat.paquet.recupererCarte( "carre", "rouge", true); 
		//Carte triangleBleuPlein = plat.paquet.recupererCarte("triangle", "bleu", true);

		Carte triangleVertPlein = plat.paquet.recupererCarte("triangle", "vert", true);
		Carte rondVertPlein = plat.paquet.recupererCarte("rond", "vert", true);
		//Carte carreVertVide = plat.paquet.recupererCarte("carre", "vert", false);
		//Carte rondRougePlein = plat.paquet.recupererCarte( "rond", "rouge", true); 
		//Carte carreBleuPlein = plat.paquet.recupererCarte("carre", "bleu", true);

		//Carte triangleVertVide = plat.paquet.recupererCarte("triangle", "vert", false);
		//Carte rondBleuPlein = plat.paquet.recupererCarte("rond", "bleu", true);
		//Carte rondBleuVide = plat.paquet.recupererCarte("rond", "bleu", false);
		//Carte carreBleuVide = plat.paquet.recupererCarte( "carre", "bleu", false); 
		//Carte carreVertPlein = plat.paquet.recupererCarte("carre", "vert", true);
		...
		plat.poserUneCarte(rondRougeVide, 2, 1);
		//plat.poserUneCarte(carreRougeVide, 2, 0);
		//plat.poserUneCarte(carreRougePlein, 3, 0);
		...
		Iterator<Joueur> it = plat.listeJoueur.iterator();
		...
		/*System.out.println("nombre de carte dans le paquet : " + plat.paquet.getNombreDeCarte());
		System.out.println("nombre de carte en jeu : " + plat.getNombreDeCarteEnJeu());
		Carte carte = plat.paquet.getRandomCarte();
		System.out.println(carte.toString());
		plat.poserUneCarte(carte, 2, 2);
		System.out.println("nombre de carte dans le paquet : " + plat.paquet.getNombreDeCarte());
		System.out.println("nombre de carte en jeu : " + plat.getNombreDeCarteEnJeu());
		...
		plat.poserUneCarte(rondRougeVide, 1, 0);
		plat.poserUneCarte(carreRougeVide, 2, 0);
		plat.poserUneCarte(carreRougePlein, 3, 0);
		plat.poserUneCarte(triangleBleuPlein, 4, 0);

		plat.poserUneCarte(triangleVertPlein, 0, 1);
		plat.poserUneCarte(rondVertPlein, 1, 1);
		plat.poserUneCarte(carreVertVide, 2, 1);
		plat.poserUneCarte(rondRougePlein, 3, 1);
		plat.poserUneCarte(carreBleuPlein, 4, 1);
		...
		j.carteVictoire = carteVictoire;

		plat.acceptVisit(v);
		int score = v.compterLesPointsAlternatif(j);
		...
		//Test bougerUneCarte
		/*Carte carte = plat.paquet.getRandomCarte();
		plat.poserUneCarte(carte, 0, 0);
		plat.montrerLesCartesPosees();
		plat.bougerUneCarte(carte, 1, 1);
		plat.montrerLesCartesPosees();*/
